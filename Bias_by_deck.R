...
pdf(file="Bias_by_deck.pdf",
    ...
    pushViewport(viewport(width=1.0,height=1.0,x=0.0,y=0.0,
	  ...
	  pushViewport(dataViewport(seq_along(filters),c(-10,10),clip='off'))
	     grid.xaxis(at=seq_along(filters),label=filters,main=T)
	     grid.text('Deck ID',y=unit(-3,'lines'))
	     grid.yaxis(main=T)
