# SLP bias in deck 248 - variation with latitude and temperature
...
for(t in seq_along(t.x)) {
   ...
   mn[t]<-mean(diffs[w],na.rm=TRUE)
   se[t]<-sd(diffs[w],na.rm=TRUE)/sqrt(length(w))
}

pdf(file="248_by_at.pdf",
